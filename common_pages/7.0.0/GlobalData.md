...
layout: default
---
The way that much of the data used by the simulator changed April 2022

This reduced the number of parameters passed through methods.

Background
==========
In the past data objects where indirectly held by the simulator and passed into 
...
Some additional data was injected into method as needed based on customized injector code.

This often made it very hard to track where the data came originally came from
and where it was actually used
It also made it hard to know if the various copies of the data where all 
...
unsupported ways leading to hard to debug errors.

There was a time between releases 6.0 and 7.0 where the data was held bu ASB 
and passed directly into the algorithm but that version is no longer supported. 

Quick Usage
=====
To access any global data just call the appropriate DataView class method.

For example to access the Transciever do:
~~~
SpynakkerDataView.get_transceiver()
~~~
or for None PyNN users do:
~~~
...
New data should be based back to the simulator which should then add it.

Model
=====

The global data is now held by a series of DataModel classes that are designed 
to be singletons so that only a single copy of each data items exists at a time.
These classes should not be accessed directly.
There are also a number fo status enums which are also designed for internal
(within Views) usage only.

The data is accessible thought the DataView classes which provide all the 
required class methods for accessing the data on demand.
There is a DataView at each code level with each level inheriting all the method from previus levels.
Graph Front Users should therefor always use FecDataView, while PyNN users should use SpynnakerDataView.
...
The only place Writer Objects should be created is in AbstractSynnakerBase or in unit tests.

Gnerally (except in unittest) data is added to the model by returning the value to ASB which then adds it.
Where it is reasonable for users or algorithm to add data directly the Views will have methods for doing so.
These View methods may included safety code to prevent unexpected changes.

The View Methods
================
The first word in a View method gives an indication of how the method reacts.
The remaining words say on what data the emthod acts.

The best and most uptodate place for details of each method is its code docs.

check...
-------
check... methods are designed for safety code.

They will raise an exception in unexpected situation.
...
(If by some change a get method is found to return the wrong data 
the correct fix is to improved the validation in the Writer class)
...
Where code needs to know if data is available as "has" methods has or should be added.

Some get methods will create a mocked value if called while in unitest mode.
...
Unittest should not count on a specific Object being returned 
but only on the Type of Object being returned.

As well direct access to data object there are also semenatic sugar get methods.
These will typically be where the same pattern of View.getFoo().bar was used several places.
Sometimes the behaviour of the semantic sugar method may be different for edge case.
These should be documented in the docs.
...
except that instead of returning a None it raises an Exception x, y are invalid.
...
They typically include safety code they thay can not be called at an unexpected time.
For example the add_vertex method will raise an Exception if called during run.

has...
---
The has methods return True if the corrosponding get method would 
return a value rather than raise an Exception.

If the get method would mock a value in unitest mode, 
the has method returns True in unittest mode even if the underlying Object has not yet been created.

For example has_machine() return if there is a machine Object saved 
but will also return True while testing as it knows it can create a Virtual Machine if reqested to do so.

has_time_step

In theory every get method could have a matching has method.
In pratice has methods have only been created where there is a need.
...
There main purpose is to protect data Object that are changed over time by only exposing their data through iterators.
...
semantic_sugar
--------------
To keep the name the same as before these methods don't follow the normal naming paattern
...
the methods that access the Application graph no longer have the term "Applcation"

These graph objects are not directly accessible. Instead use the iterate and get methods to access the graph's data and the add method to change the graph.
...
As well as get_runtime_best_graph for methods that need a graph but dont mind which one.
...
In the unlikely case that the work to remove the clones graphs does not make it in,
we reserve the right to replace the get_runtime_(machine)_graph methods with ieterators and add methods.

machine_time_step
-----------------
machine_time_step has been renamed simulation_time_step to highlight that it 
is the rate the rate the simulation mathematically thinks it is running at.
...
--------------------------------
data n timesteps now available as View.get_max_run_time_steps()
This better reflects that the value says the maximum time_stps a run could be before auto pause reseume kicks in.
Mainly used to size recording regions

ExtendedMachine
---------------
As ASB now controls when alogorithms are run the distinction between Mmachine and extended machine has been dropped.
...
(app/system)provenance_file_path
--------------------------------
(app/system)provenance_file_paths have been renamed get_(app/system)provenance_dir_path as it is actually a directory.
...
Inject_items
============
This has been completely remove.
...
Why is View data sent as a parameter is some cases
==================================================
...
------------------------------------
There are some case where the same method or class is used both for the data as held in the View and 
sometimes for different data or reduce set of this data. 

For example IOBufExtractor normally reads all the data 
but in some cases for example in emergency_recover only reasons for some executable_targets.
...
That way only places that want none view data need to worry about that data.

The Transceiver methods mainly still require an app_id.
This is partly because in some cases like get_multicast_routes A none app_id already has the meaning any
so could not mean the default.

Method not yet converted
-------------------------
There are still many cases, especially for local methods that data obtained from a View is passed.
...
- transceiver removed
    - HasSynapses.get_connections_from_machine no longer needs a transciever
- placements removedd
...
Unit Testing
===========
One disadvantage of the Global Data approach that it does make unittest harder to write.

Instead of creating the required objects and passing them into a method,
the untitest now has to add the Objects for the method to then get them from a View.
...
The unittest of most View methods will show how to add data to the view.

...DataWriter.mock()
--------------------
Unittests can obtain a Writer object uning the call ...DataWriter.mock().
...
Therefor each unittest should only create one Writer.
This includes the one line ...DataWriter().set... calls.

unittest_setup() at the PACMAN level and above calls DataWriter.mockso that any test that calls unittest_setup() 
is guranteed to be in mock state with cleared data.
...
Machine
-------
There is no need to create a Machine object unless the test needs a specif Machine.
...
------------------------------
The iter method of Placements object was removed as never used.
It was also confusing as to what it actually itered over.
