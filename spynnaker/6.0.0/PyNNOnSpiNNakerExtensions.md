...
If you are writing new neural models, synapse models or plasticity models, it is likely that you will need to compile C code to run on SpiNNaker.  If you are using a Jupyter notebook then this C code will already be available to you via the Terminal window so there is no need to download and install anything; if you wish to do a local install on your own machine, then [follow these instructions](/common_pages/6.0.0/CDevelopmentForSpiNNaker.html) to install a C development environment, and if your code is then to make use of the sPyNNaker neural modelling code base (which avoids the need to rewrite much of the necessary code):

1. Download the sPyNNaker source code as a [zip](https://github.com/SpiNNakerManchester/sPyNNaker/archive/6.0.0.zip) or [tar.gz](https://github.com/SpiNNakerManchester/sPyNNaker/archive/6.0.0.tar.gz)
...
* [Tutorial: Adding new Neural Models](NewNeuronModels6.0-LabManual.pdf)
