...
If True is assumes the BMP ip addresss is one less than the [Ethernet](machine_name)
...
Testing Option.
Overrides auto detection if an [Input output database](option) is needed.
### <a name="create_routing_info_to_neuron_id_mapping"></a> create_routing_info_to_neuron_id_mapping
Default: __True__

Adds data abuout the routing to the [Input output database](option)
...
Database file to hold data read back from the machine including [Provenance](option).
...
Format to use to [draw](option) a large graph
...
If provenace is being [written to the database](option)
this will liit the amount that will be [Logged](instantiate)
### <a name="read_graph_provenance_data"></a> read_graph_provenance_data
Default: __Debug__

Reads graph provenance and writes it into the [database](option)
### <a name="read_placements_provenance_data"></a> read_placements_provenance_data
Default: __Debug__

Reads placements provenance and writes it into the [database](option)
### <a name="read_profile_data"></a> read_profile_data
Default: __Debug__

Reads profile provenance and writes it into the [database](option)
### <a name="read_provenance_data_on_end"></a> read_provenance_data_on_end
Default: __Debug__

Reads provenace data at the end and writes it into the [database](option)
### <a name="read_router_provenance_data"></a> read_router_provenance_data
Default: __Debug__

Reads router provenance and writes it into the [database](option)
...
Timings are always saved in the [global provenance database](option)
They may laso be [logged](display_algorithm_timings).
...
Will be written with [System iobuff](option)
...
Reads other provenance and writes it into the [database](option)
...
Note: The [database](option) is what is actually used for loading the data specs.
...
Setting for the [Energy Monitor](option)
...
The number of smaples taken in each recording of the [Energy Monitor](option).
### <a name="sampling_frequency"></a> sampling_frequency
Default: __10__

How often [Energy Monitor](option) will sample, in microseconds
# <a name="Java"></a> Java
